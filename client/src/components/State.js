...
function PostEvent(){
    ...
    const cloudinaryUpload = async () => {
        if(eventId === "" || actDate === "" || actDateTo === "" || evntDesc === "" || noOfStud === "" || evntPic1 === "" || evntPic2 === "" || evntPic3 === "" || evntPic4 === "" || evntCerti === "" || evntPstr === "" || studSheet === "" || inputList === ""){
        ...
        data.append("file", evntPic1[0])
        data.append("upload_preset", "sgp-post")
        data.append("cloud_name", "dkoj7svtw")
        await fetch("https://api.cloudinary.com/v1_1/dkoj7svtw/image/upload", {
            method: "post",
            body: data
        })
        .then( res => res.json())
        ...
        //SPK CV & PHOTO
        .then(async ()=>{
            inputList.map(async (spk, i) => {
                //CV
                const data = new FormData()
                data.append("file", spk.spkCV[0])
                data.append("upload_preset", "sgp-post")
                data.append("cloud_name", "dkoj7svtw")
                await fetch("https://api.cloudinary.com/v1_1/dkoj7svtw/image/upload", {
                    method: "post",
                ...
                .then( res => res.json())
                .then(data1 => {
                    let spkURL = inputListUrl
                    console.log(data1.url)
                    spkURL.push(data1.url)
                    setInputListUrl(spkURL)
                })
                .then(async ()=>{
                    //PHOTO
                    const data = new FormData()
                    data.append("file", spk.spkPhoto[0])
                    data.append("upload_preset", "sgp-post")
                    data.append("cloud_name", "dkoj7svtw")
                    await fetch("https://api.cloudinary.com/v1_1/dkoj7svtw/image/upload", {
                        method: "post",
        ...
        <div class="grid-container">
            ...
            <div>
                <div className="sub-btn">
                    <div class="row">
                        { !loading && 
                        <input className="submit" onClick={ cloudinaryUpload } type="submit" value="Upload" />
                        }
                        { loading && 
